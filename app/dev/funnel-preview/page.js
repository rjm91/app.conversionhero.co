...
const STEP_LABELS = ["Outage frequency", "Fuel", "Home size", "Location", "Contact"]

function CertBadge() {
  return (
    <div className="cert-badge" aria-hidden="true">
      <svg viewBox="0 0 28 28" width="28" height="28">
        <path d="M14 2 L17 5 L21 4 L22 8 L26 10 L24 14 L26 18 L22 20 L21 24 L17 23 L14 26 L11 23 L7 24 L6 20 L2 18 L4 14 L2 10 L6 8 L7 4 L11 5 Z"
              fill="none" stroke="var(--brand)" strokeWidth="1.2" strokeLinejoin="round"/>
        <path d="M9 14 L13 18 L19 10" stroke="var(--brand)" strokeWidth="1.6" fill="none" strokeLinecap="round" strokeLinejoin="round"/>
      </svg>
    </div>
  )
}

function ProgressRail({ step, total, labels }) {
  return (
    <div className="rail">
      <div className="rail-brand">
        <img className="rail-logo" src="https://mbodzggsefkpesqcxskv.supabase.co/storage/v1/object/public/funnel-assets/clients/ch014/synergy-logo.svg" alt="Synergy Home" />
      </div>
      <ol className="step-list">
        {labels.map((label, i) => (
          <li key={i} className={`step-item ${i === step ? 'active' : ''} ${i < step ? 'done' : ''}`}>
            <div className="step-dot-wrap">
              <div className={`step-dot ${i === step ? 'active' : ''} ${i < step ? 'done' : ''}`}>
                {i < step ? (
                  <svg width="10" height="10" viewBox="0 0 10 10" fill="none">
                    <path d="M2 5.2L4 7.2L8 2.8" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
                ) : <span>{i + 1}</span>}
              </div>
            </div>
...
function ZipStep({ zip, setZip, city, setCity, error, setError }) {
  function onChange(e) {
    ...
    if (v.length === 5) {
      const c = KY_ZIPS[v]
      if (c) { setCity(c); setError('') }
      else { setCity(''); setError('Outside our service area — call us to confirm') }
    } else { setCity(''); setError('') }
  }
  return (
    <div className="zip-field-wrap">
      <label className="field-label" htmlFor="zip-input">ZIP code</label>
...
function Survey({ theme }) {
  ...
  const canAdvance = useMemo(() => {
    ...
    if (step === 3) return zip.length === 5 && !!city
    if (step === 4) return !!(contact.name && contact.email && contact.phone)
    return false
  }, [step, reason, fuel, size, zip, city, contact])
  ...
  function scheduleAdvance() {
    ...
    advanceTimer.current = setTimeout(() => { setDirection(1); setStep(s => Math.min(total - 1, s + 1)) }, 600)
  }
  function next() {
    clearTimeout(advanceTimer.current)
  ...
  return (
    <div className="survey-shell survey-shell--centered" data-theme={theme}>
        ...
        <section className="card">
                  ...
                  <div className="opt-grid">
                    {REASONS.map((o, i) => (
                      <OptionTile key={o.id} option={o} index={i} selected={reason === o.id}
                        onClick={() => { setReason(o.id); scheduleAdvance() }} />
                    ))}
                  </div>
                </>
              )}
                  ...
                  <div className="opt-grid">
                    {FUEL.map((o, i) => (
                      <OptionTile key={o.id} option={o} index={i} selected={fuel === o.id}
                        onClick={() => { setFuel(o.id); scheduleAdvance() }} />
                    ))}
                  </div>
                </>
              )}
                  ...
                  <div className="opt-grid">
                    {SIZE.map((o, i) => (
                      <OptionTile key={o.id} option={o} index={i} selected={size === o.id}
                        onClick={() => { setSize(o.id); scheduleAdvance() }} />
                    ))}
                  </div>
                </>
              )}
          ...
          <footer className="card-foot">
            ...
            <button type="button" className="btn-primary" onClick={next} disabled={!canAdvance}>
              {step === total - 1 ? 'Get my free quote' : 'Continue'}
