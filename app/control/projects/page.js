...
const STATUS_META = {
  active:    { label: 'Active',    dot: 'bg-green-500', cls: 'bg-green-50 text-green-700 dark:bg-green-500/10 dark:text-green-400' },
  on_hold:   { label: 'On Hold',   dot: 'bg-yellow-400', cls: 'bg-yellow-50 text-yellow-700 dark:bg-yellow-500/10 dark:text-yellow-400' },
  completed: { label: 'Completed', dot: 'bg-indigo-400', cls: 'bg-blue-50 text-blue-700 dark:bg-blue-500/10 dark:text-blue-400' },
  archived:  { label: 'Archived',  dot: 'bg-gray-400', cls: 'bg-gray-100 text-gray-500 dark:bg-white/10 dark:text-gray-400' },
}
...
export default function ProjectsPage() {
  ...
  const grouped = useMemo(() => {
    ...
    projects.forEach(p => {
      const s = g[p.status] ? p.status : 'active'
      g[s].push(p)
    })
    return g
  }, [projects])
  ...
  const doneTasks = expandedTasks.filter(t => t.status === 'done').length
  ...
  return (
    <div className="p-8">
      ...
      <div className="grid grid-cols-4 gap-3 mb-6">
        {[
          { label: 'Total Projects', val: stats.total, color: 'text-gray-900 dark:text-white' },
          { label: 'Active', val: stats.active, color: 'text-green-500' },
          { label: 'On Hold', val: stats.on_hold, color: 'text-yellow-400' },
          { label: 'Completed', val: stats.completed, color: 'text-indigo-400' },
        ].map(s => (
      ...
      {loading ? <p className="text-sm text-gray-400">Loading...</p> : (
        <div className="flex flex-col gap-3">
          {STATUS_ORDER.map(status => {
            ...
            return (
              <div key={status} className="bg-white dark:bg-[#171B33] rounded-xl border border-gray-100 dark:border-white/5 overflow-hidden">
                {/* Group header */}
                <button onClick={() => toggleGroup(status)}
                  className="w-full flex items-center gap-3 px-5 py-3.5 hover:bg-gray-50 dark:hover:bg-white/[0.02] transition">
                  ...
                  <div className="border-t border-gray-100 dark:border-white/5">
                    {group.map(p => {
                      ...
                      return (
                        <div key={p.id}>
                          {/* Project row */}
                          <div className={`flex items-center gap-3 px-5 py-3 pl-14 cursor-pointer transition border-b border-gray-50 dark:border-white/[0.03] hover:bg-gray-50 dark:hover:bg-white/[0.02] ${isExpanded ? 'bg-blue-50/50 dark:bg-blue-500/[0.04]' : ''}`}
                            ...
                            <div className="flex items-center gap-2 flex-shrink-0">
                              ...
                              {tasks_.length > 0 && (
                                <>
                                  <div className="w-14 h-1.5 bg-gray-200 dark:bg-white/10 rounded-full overflow-hidden">
                              ...
                              <button onClick={e => { e.stopPropagation(); setEditProject(p) }}
                                className="p-1 rounded hover:bg-gray-200 dark:hover:bg-white/10 transition text-gray-400 hover:text-gray-600 dark:hover:text-gray-200"
                                title="Edit project">
                              ...
                              {tasksLoading ? (
                                ...
                                <div className="px-14 py-4 flex items-center gap-3">
                                  ...
                                  <button onClick={() => { setAddingTaskFor(p.id); setTaskForm(emptyTask) }}
                                    className="text-xs text-gray-400 border border-dashed border-gray-300 dark:border-white/10 px-3 py-1 rounded hover:text-blue-500 hover:border-blue-500 transition">
                                    + Add task
                                  </button>
                                </div>
                              ) : (
                                <>
                                  ...
                                  {addingTaskFor === p.id ? (
                                    ...
                                    <div className="px-20 py-2">
                                      <button onClick={() => { setAddingTaskFor(p.id); setTaskForm(emptyTask) }}
                                        className="text-[11px] text-gray-400 border border-dashed border-gray-300 dark:border-white/10 px-3 py-1 rounded hover:text-blue-500 hover:border-blue-500 transition">
                                        + Add task
                                      </button>
                                    </div>
                                  )}
      ...
      {editingTask && (
        <>
          ...
          <div className="fixed top-0 right-0 h-full w-[380px] bg-white dark:bg-[#1a1f3a] shadow-2xl z-40 flex flex-col border-l border-gray-100 dark:border-white/5">
