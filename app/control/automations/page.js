...
export default function AutomationsPage() {
  ...
  async function load() {
    setLoading(true)
    const res = await fetch('/api/agency-automations', { cache: 'no-store' })
  ...
  async function toggleEnabled(rule) {
    ...
    await fetch(`/api/agency-automations/${rule.id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function saveEditing() {
    ...
    const res = await fetch(`/api/agency-automations/${editing.id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function deleteEditing() {
    ...
    await fetch(`/api/agency-automations/${editing.id}`, { method: 'DELETE' })
  ...
  async function sendTest(rule) {
    ...
    setTestResult(null)
    try {
      const res = await fetch(`/api/agency-automations/${rule.id}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div className="p-8 relative">
      <div className="mb-6">
        ...
        <p className="text-gray-400 text-sm mt-0.5">Trigger actions when things happen in your agency.</p>
      </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <p className="text-sm text-gray-500 dark:text-gray-400">
              Email your team when a new lead comes in.
            </p>
            <button
          ...
          <div className="text-xs text-gray-400 dark:text-gray-500 space-y-1">
            <p><span className="font-medium text-gray-500 dark:text-gray-400">Contact:</span> <code>{'{{first_name}}'}</code> <code>{'{{last_name}}'}</code> <code>{'{{email}}'}</code> <code>{'{{phone}}'}</code> <code>{'{{company}}'}</code> <code>{'{{city}}'}</code> <code>{'{{state}}'}</code> <code>{'{{zip_code}}'}</code></p>
          ...
          <div className="fixed top-0 right-0 bottom-0 w-full max-w-md bg-white dark:bg-[#0f1117] z-40 shadow-2xl flex flex-col border-l border-gray-200 dark:border-white/10">
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
              ...
              <div>
                ...
                />
                <p className="text-[11px] text-gray-400 mt-1">Sent from notifications@conversionhero.co</p>
              </div>
