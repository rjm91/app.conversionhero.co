...
export async function POST(request) {
  ...
  const systemPrompt = [
    ...
    pageContext === 'Agreement Builder'
      ? `AGREEMENT BUILDER MODE: The user is building a client agreement. When they describe a deal in plain language, call proposeAgreement with your best mapping of the fields — this fills the on-screen builder for them to review and send. It does NOT save or send anything. Packages: pilot ($1000/mo, 8 videos), starter ($1550/mo, 13), growth ($2450/mo, 21), pro ($3750/mo, 34), or "custom" (set customName, customScope, and customPrice — use customPrice 0 for setup-only or revenue-share-only deals). Fees: setupFee (one-time), adOn+adPct (ad-spend commission), revOn+revPct+revStartDays (revenue share = % of collected revenue starting N days from today — for "after 120 days" use revStartDays:120). term is one of "Month-to-month","30 days","60 days","90 days","4 months","6 months","12 months", or "Custom…" with termCustom. After proposing, say "I've drafted the agreement — review it above and accept to fill the builder." Today is ${todayISO}.`
      : '',
