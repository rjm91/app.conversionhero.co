...
export async function POST(request, { params }) {
  // Test send: POST /api/client-automations/[id] with { action: 'test' }
  try {
    const { id } = await params
    ...
    const { dispatchClientEvent } = await import('../../../../lib/automations.js')
    await dispatchClientEvent(rule.client_id, 'lead.created', {
      first_name: 'Test',
      last_name: 'Lead',
      email: 'test@example.com',
      phone: '555-0100',
      company: '',
      agency_funnels: { name: 'Test Funnel', slug: 'test' },
    })
