...
export const TOOL_DEFINITIONS = [
  ...
  },
  {
    ...
    description: 'Agency-only. Get payment totals from client_payments table. Can filter by client_id. Returns total amount collected, transaction count, and breakdown by client. Use for questions about payments, revenue, cash collected, or "which client has paid the most".',
    input_schema: {
      type: 'object',
      properties: {
...
export async function runTool({ name, input, user }) {
  ...
  if (name === 'getPaymentSummary')   return getPaymentSummary({ clientId: input.clientId, user })
...
async function getPaymentSummary({ clientId, user }) {
  const db = adminClient()
  const { data: profile } = await db.from('profiles').select('role').eq('id', user.id).single()
  if (profile?.role !== 'agency_admin' && profile?.role !== 'agency_standard') return { error: 'Agency access required' }
  let query = db.from('client_payments').select('client_id, amount')
