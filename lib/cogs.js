...
// Line items for a stored order row — the client_order_items child rows
// (embedded by PostgREST as `client_order_items`, or pre-attached as `items`).
export function orderItems(order) {
  return order?.items || order?.client_order_items || []
}

// Convenience: COGS for a stored order row.
export function orderRowCogs(order, skuIndex, book) {
  return orderCogs(orderItems(order), skuIndex, book)
}
