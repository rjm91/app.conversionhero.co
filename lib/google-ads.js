...
export async function getGoogleAdsAccessToken() {
  ...
  const res = await fetch('https://oauth2.googleapis.com/token', {
    method: 'POST',
    // CRITICAL: Next.js patches global fetch and will cache/replay this POST,
    // serving a stale (expired) access token → Google returns invalid_token / 401.
    // no-store forces a fresh token exchange every call.
    cache: 'no-store',
...
export async function exchangeCodeForGoogleAdsToken(code, callbackUrl) {
  const res = await fetch('https://oauth2.googleapis.com/token', {
    method: 'POST',
    cache: 'no-store', // see note in getGoogleAdsAccessToken — never let Next cache token exchanges
    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
    body: new URLSearchParams({
      client_id:     process.env.GOOGLE_ADS_CLIENT_ID,
      client_secret: process.env.GOOGLE_ADS_CLIENT_SECRET,
