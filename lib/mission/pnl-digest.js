...
export function digestModel(clientName, clientId, date, m = {}, tz = DEFAULT_TZ, extras = {}) {
  ...
  // One-line read: the day's net + which platform carried it. The whole line
  // is the {{lede}} token; {{platform_note}} is just the led/trailed sentence
  // so a custom template can compose its own opening around it.
  let platformNote = ''
  if (ch && meta.spend > 0 && goog.spend > 0) {
    ...
    if (mR != null && gR != null) {
      ...
      platformNote = `${lead[0]} led (${xR(lead[1])}); ${lag[0]} ${lag[1] < 1 ? 'ran below breakeven' : 'trailed'} (${xR(lag[1])}).`
    }
  }
  // Name the day honestly: the morning digest covers YESTERDAY's completed
  // day; "so far today" only when the digest is generated for the current day.
  const dayWord = date === dayInTz(tz, -1) ? 'Yesterday' : date === dayInTz(tz, 0) ? 'So far today' : prettyDate(date)
  const lede = `${dayWord}: ${$(cm)} net on ${$(m.grossSales)} gross.${platformNote ? ' ' + platformNote : ''}`
