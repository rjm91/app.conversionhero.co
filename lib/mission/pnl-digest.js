...
export function formatDigest(clientName, clientId, date, m = {}) {
  ...
  // Margin measured against GROSS sales, to match the Overview (Jason's basis).
  const marginGross = m.grossSales ? m.grossProfit / m.grossSales : m.profitMargin
  const summary = `${clientName} — Daily P&L · ${prettyDate(date)} · Net ${$(m.netSales)} · GP ${$(m.grossProfit)} (${pct(marginGross)})`
  return {
    ...
    blocks: [
      ...
      { type: 'section', fields: [
        ...
        { type: 'mrkdwn', text: `*Gross Profit*\n${$(m.grossProfit)}  (${pct(marginGross)} of gross)` },
