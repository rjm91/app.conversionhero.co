...
// Merchandise money from a client_orders row (real columns since the
// shopify_data blob was retired; falls back to sale_amount for rows that
// never captured subtotal/discounts).
export function orderMoney(row) {
  const has = row.subtotal != null || row.discounts != null
  if (has) {
