...
export async function fetchMissionData(clientId, start, end) {
  ...
  return {
    ...
    // Raw per-day rows kept for the daily time series — the campaign tables
    // are already daily in the DB; no new schema needed for date bucketing.
...
export function computeMission({ orders, google, meta, googleDaily = [], metaDaily = [], mfg, start, end }) {
  ...
  // Daily time series — orders bucketed by local calendar day, spend from the
  // per-day campaign rows. This is what lets "last 7 days" style questions
  // work without any new DB tables.
  const dailyMap = {}
  const bucket = (d) => (dailyMap[d] = dailyMap[d] || { date: d, revenue: 0, orders: 0, cogs: 0, spend: 0 })
  for (const o of orders) {
    const dt = new Date(o.created_at)
    const p = (n) => String(n).padStart(2, '0')
    const d = `${dt.getFullYear()}-${p(dt.getMonth() + 1)}-${p(dt.getDate())}`
    const b = bucket(d)
    b.revenue += Number(o.sale_amount) || 0
    b.orders += 1
    b.cogs += cogsOf(o)
  }
  for (const r of googleDaily) bucket(r.date).spend += r.spend
  for (const r of metaDaily) bucket(r.date).spend += r.spend
...
export function askContext(clientName, m, range) {
  return {
    ...
    // Per-day series (ISO date, whole dollars) — lets the model answer any
    // sub-window question ("last 7 days", "this week vs last") by arithmetic.
    daily: (m.daily || []).map(d => ({
