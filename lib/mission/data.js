...
export async function fetchMissionData(clientId, start, end) {
  ...
  const [ordersRes, googleRes, metaRes, mfgRes, clientRes, firstsRes] = await Promise.all([
    ...
    supabase.from('client').select('client_name, settings, is_ecom').eq('client_id', clientId).single(),
  ...
  return {
    ...
    pnlConfig: {
      ...
      aovRedBelow: clientRes.data?.settings?.aov_red_below, aovGreenAbove: clientRes.data?.settings?.aov_green_above,
    },
