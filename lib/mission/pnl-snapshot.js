...
export async function snapshotDailyPnl(db, clientId, start, end, config = {}) {
  ...
  // Per-channel daily rows (client_channel_daily_pnl): one row per channel
  // that had revenue OR ad spend that day — paid (Meta/Google) and organic
  // (Direct, Klaviyo, Shop, Draft Order, …). Facts only; ratios derive.
  const channelRows = []
  const rows = Object.entries(byDay).filter(([date]) => date >= start && date <= end).map(([date, g]) => {
    ...
    p.channels = ch
    // All-channel rollup for client_channel_daily_pnl.
    const byCh = {}
    for (const o of g.orders) {
      const c = deriveChannel(o)
      const t = (byCh[c] = byCh[c] || { gross: 0, net: 0, discounts: 0, refunds: 0, orders: 0, newOrders: 0, cogs: 0 })
      const mm = orderMoney(o)
      t.gross += mm.gross; t.net += mm.net; t.discounts += mm.discounts; t.refunds += mm.refunds; t.cogs += cogsOf(o)
      if (mm.net > 0) { t.orders += 1; if (firstIds.has(o.lead_id || o.order_id)) t.newOrders += 1 }
    }
    if (g.meta > 0) byCh.Meta = byCh.Meta || { gross: 0, net: 0, discounts: 0, refunds: 0, orders: 0, newOrders: 0, cogs: 0 }
    if (g.google > 0) byCh.Google = byCh.Google || { gross: 0, net: 0, discounts: 0, refunds: 0, orders: 0, newOrders: 0, cogs: 0 }
    for (const [channel, v] of Object.entries(byCh)) {
      channelRows.push({
        client_id: clientId, day: date, channel,
        gross_revenue: round(v.gross), net_revenue: round(v.net),
        discounts: round(v.discounts), refunds: round(v.refunds),
        orders: v.orders, new_orders: v.newOrders, cogs: round(v.cogs),
        spend: channel === 'Meta' ? round(g.meta) : channel === 'Google' ? round(g.google) : 0,
        computed_at: now,
      })
    }
    return {
  ...
  if (rows.length) {
    ...
    // Replace the recomputed days' channel rows (delete-then-insert so a
    // channel that dropped to zero disappears instead of lingering).
    const days = rows.map(r => r.date)
    const { error: de } = await db.from('client_channel_daily_pnl')
      .delete().eq('client_id', clientId).in('day', days)
    if (de) throw new Error(de.message)
    for (let i = 0; i < channelRows.length; i += 500) {
      const { error: ce } = await db.from('client_channel_daily_pnl').insert(channelRows.slice(i, i + 500))
