...
export async function snapshotDailyPnl(db, clientId, start, end, config = {}) {
  const costPerLabel = config.costPerLabel != null ? config.costPerLabel : 25
  ...
  for (const o of data.orders) bucket(dayKey(o.created_at)).orders.push(o)
  ...
  const rows = Object.entries(byDay).map(([date, g]) => {
