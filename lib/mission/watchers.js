...
export function buildFindings(m) {
  ...
  for (const c of live) {
    ...
    if (c.trueRoas < 1) {
      ...
      const winner = live.filter(x => x !== c && x.trueRoas > 1.5).sort((a, b) => b.trueRoas - a.trueRoas)[0]
      f.push({
  ...
  // 2. Scale headroom: clear winner well above breakeven
  const winners = live.filter(c => c.trueRoas != null && c.trueRoas >= 2 && c.spend >= 200).sort((a, b) => b.trueRoas - a.trueRoas)
  if (winners[0]) {
    const w = winners[0]
    f.push({
