...
export async function fetchServerMissionData(db, clientId, start, end) {
  ...
  const [ordersRes, googleRes, metaRes, materialsRes, skusRes, clientRes] = await Promise.all([
    ...
    fetchAllRows((from, to) => db.from('client_orders')
      .select('lead_id:order_id, order_id, sale_amount, utm_campaign, utm_source, utm_medium, utm_content, created_at, order_name, fulfillment_status, shopify_channel, subtotal, discounts, refunds, first_utm_source, first_utm_medium, first_utm_campaign, last_utm_source, last_utm_medium, last_utm_campaign, client_order_items(sku, qty, title)')
