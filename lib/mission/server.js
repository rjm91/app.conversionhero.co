...
export async function fetchServerMissionData(db, clientId, start, end) {
  ...
  const [ordersRes, googleRes, metaRes, materialsRes, skusRes, clientRes] = await Promise.all([
    db.from('client_lead')
      ...
      .eq('client_id', clientId).like('lead_id', 'shopify_%')
      .gte('created_at', dayStartISO).lte('created_at', dayEndISO),
    db.from('client_yt_campaigns').select('*').eq('client_id', clientId)
    ...
    db.from('client_skus').select('parent_sku, size, bom').eq('client_id', clientId),
  ...
  return {
    ...
    mfg: { materials: materialsRes.data || [], skus: skusRes.data || [] },
