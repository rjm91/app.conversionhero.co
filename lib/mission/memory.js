...
export async function recallMemories(clientId, query, limit = 6) {
  try {
    ...
    if (error) return []
    // Floor calibrated to voyage-3.5's distribution: a genuinely-relevant hit
    // (e.g. "peak season is Q4" ↔ "when to push ad spend") scores ~0.32, while
    // unrelated content sits below ~0.25. Top-K + this floor; the agent is told
    // these are "what you remember" and ignores any that don't fit.
    return (data || []).filter(m => m.similarity >= 0.25)
  } catch { return [] }
}
