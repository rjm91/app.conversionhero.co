...
export function useTerminalHistory(surface) {
  ...
  // Rename a session — optimistic local update + fire-and-forget PATCH.
  const renameSession = useCallback(async (id, title) => {
    if (!id) return
    const clean = (title == null ? '' : String(title)).trim().slice(0, 80)
    setSessions(prev => prev.map(s => s.id === id ? { ...s, title: clean || 'New conversation' } : s))
    if (!surface) return
    try {
      await fetch('/api/terminal-chat', {
        method: 'PATCH', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ surface, session_id: id, title: clean }),
      })
    } catch { /* invisible */ }
  }, [surface])

  return { ready, sessions, sessionId, messages, saveTurn, newSession, loadSession, renameSession }
}
