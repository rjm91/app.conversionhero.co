...
export async function getMetaConnection(clientId) {
  const { data } = await admin()
    ...
    .select('client_id, ad_account_id, access_token, app_secret, connection_method')
...
function withProof(params, conn) {
  // OAuth connections belong to the ConversionHero Meta app, whose secret
  // remains server-side. Legacy/manual connections keep their per-row secret
  // behavior so ShieldTech and other existing clients are unchanged.
  const secret = conn.app_secret || (conn.connection_method === 'facebook_oauth' ? process.env.META_FACEBOOK_APP_SECRET : null)
  if (secret) {
    const proof = crypto.createHmac('sha256', secret).update(conn.access_token).digest('hex')
