...
// read_all_orders lifts the 60-day order window (needed for full-history geo
// backfill); it only takes effect after the store re-approves via OAuth.
export const SHOPIFY_SCOPES = 'read_orders,read_all_orders,read_customer_events'
...
export const ORDER_GQL_FIELDS = `
  ...
  billingAddress { firstName lastName address1 city province provinceCode zip country countryCodeV2 phone }
  shippingAddress { firstName lastName address1 city province provinceCode zip country countryCodeV2 phone }
  shippingLine { title }
...
// Map a Shopify order GraphQL node → a client_orders row. UTM IDs come from
// whichever visit (last preferred, first fallback) has them.
export function orderNodeToOrderRow(clientId, node) {
  ...
  // Geo: shipping address preferred (where the product went = where the
  // customer is), billing as fallback. Powers zip/city/state analysis in
  // the dashboard and the agent's query tool.
  const ship = node.shippingAddress || {}
  const bill = node.billingAddress || {}
  const geo = (k) => ship[k] || bill[k] || null
  // Money basis for the daily P&L (merchandise, excl. tax/shipping — matches
  // the client's morning report). subtotal = after discounts, before tax/ship.
  const money = (s) => s?.shopMoney?.amount != null ? Number(s.shopMoney.amount) : null
  const subtotal = money(node.subtotalPriceSet)
  const discounts = money(node.totalDiscountsSet)
  const refunds = money(node.totalRefundedSet)
  return {
    order_id:     `shopify_${numericId}`,
    client_id:    clientId,
    ...
    city:         geo('city'),
    state:        ship.provinceCode || bill.provinceCode || ship.province || bill.province || null,
    zip_code:     geo('zip'),
    sale_amount:  node.totalPriceSet?.shopMoney?.amount ? Number(node.totalPriceSet.shopMoney.amount) : null,
    source:       'shopify',
    notes:        products ? `Shopify order ${node.name}: ${products}` : `Shopify order ${node.name}`,
    ...
    // Shopify fields as REAL COLUMNS (the old shopify_data blob is gone;
    // line items live in client_order_items — see orderNodeToItems).
    order_name:         node.name,
    shopify_channel:    channelLabel(node.sourceName),
    ...
    country:            ship.countryCodeV2 || bill.countryCodeV2 || ship.country || bill.country || null,
    // Daily-P&L money fields (merchandise basis, excl. tax/shipping):
    //   gross = subtotal + discounts · net = subtotal − refunds
    subtotal, discounts, refunds, tax: money(node.totalTaxSet),
    // Keep both visits for attribution auditing and older-row fallback. The
    // reporting rule is last touch: top-level UTM (last preferred above), then
    // explicit last visit, then first visit.
    first_utm_source: first.source || null, first_utm_medium: first.medium || null,
    first_utm_campaign: first.campaign || null, first_utm_content: first.content || null,
    last_utm_source: last.source || null, last_utm_medium: last.medium || null,
    last_utm_campaign: last.campaign || null, last_utm_content: last.content || null,
  }
}

// Line-item rows for client_order_items (one per order line). Callers upsert
// the order row, then replace its item rows with these.
export function orderNodeToItems(clientId, node) {
  const numericId = String(node.id).split('/').pop()
