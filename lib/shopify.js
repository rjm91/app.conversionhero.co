...
export function orderNodeToOrderRow(clientId, node) {
  ...
  // Per-line SKUs (for COGS / margin matching). sku comes from the line item
  // itself (read_orders); null for custom lines or untracked add-ons.
  const lineItemRows = lineItems.map(li => ({ sku: li.sku || null, title: li.title || null, qty: li.quantity || 0 }))
